    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex align-items-center justify-content-between">
            <h6 class="m-0 font-weight-bold text-primary">
                <i class="fas fa-edit fa-fw me-1"></i> Modifier la question du quiz "{{ $quiz->title }}"
            </h6>
            <div>
        ...
        <div class="card-body">
            <form action="{{ route('admin.questions.update', $question->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('question') is-invalid @enderror" id="question" name="question" value="{{ old('question', $question->text) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                        <option value="multiple_choice" {{ old('type', $question->type) === 'multiple_choice' ? 'selected' : '' }}>Choix multiple</option>
                        <option value="true_false" {{ old('type', $question->type) === 'true_false' ? 'selected' : '' }}>Vrai/Faux</option>
                        <option value="short_answer" {{ old('type', $question->type) === 'short_answer' ? 'selected' : '' }}>Réponse courte</option>
                    </select>
                    @error('type') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                
                <div id="options-container" class="mb-3 {{ old('type', $question->type) === 'short_answer' ? 'd-none' : '' }}">
                    <label class="form-label">Options de réponse <span class="text-danger">*</span></label>
                    
                    <div id="multiple-choice-options" class="{{ old('type', $question->type) === 'true_false' ? 'd-none' : '' }}">
                        <div class="options-list">
                            @php 
                                $questionOptions = [];
                                if (old('options')) {
                                    $questionOptions = old('options');
                                } elseif ($question->options && $question->type === 'multiple_choice') {
                                    $questionOptions = json_decode($question->options, true) ?? [];
                                }
                            @endphp
                    ...
                    <div id="true-false-options" class="{{ old('type', $question->type) !== 'true_false' ? 'd-none' : '' }}">
                        <div class="form-check mb-2">
                            <input class="form-check-input" type="radio" name="correct_answer" id="option-true" value="true" 
                                {{ old('correct_answer', $question->correct_answer) === 'true' ? 'checked' : '' }} required>
                            <label class="form-check-label" for="option-true">Vrai</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="correct_answer" id="option-false" value="false" 
                ...
                <div id="correct-answer-container" class="mb-3 {{ old('type', $question->type) === 'true_false' ? 'd-none' : '' }}">
                    <label for="correct_answer" class="form-label">Réponse correcte <span class="text-danger">*</span></label>
                    <input type="text" class="form-control @error('correct_answer') is-invalid @enderror" id="correct_answer" 
                        name="correct_answer" value="{{ old('correct_answer', $question->correct_answer) }}" 
                        {{ old('type', $question->type) === 'true_false' ? 'readonly' : '' }} required>
                    <div class="form-text" id="correct-answer-help">
                        @if(old('type', $question->type) === 'multiple_choice')
                            Pour les questions à choix multiple, entrez exactement une des options ci-dessus.
                        @elseif(old('type', $question->type) === 'short_answer')
                            Entrez la réponse exacte attendue.
                        @endif
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('explanation') is-invalid @enderror" id="explanation" 
                        name="explanation" rows="3">{{ old('explanation', $question->explanation) }}</textarea>
                ...
                <div class="d-flex mt-4 justify-content-between">
                    ...
                    <form action="{{ route('admin.questions.destroy', $question->id) }}" method="POST" class="d-inline" 
                        ...
                        <button type="submit" class="btn btn-danger">
                            <i class="fas fa-trash fa-fw"></i> Supprimer la question
                        </button>
                    </form>
                </div>
            </form>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        questionType.addEventListener('change', function() {
            if (this.value === 'multiple_choice') {
                ...
                correctAnswerInput.value = document.querySelector('input[name="correct_answer"]:checked').value || 'true';
